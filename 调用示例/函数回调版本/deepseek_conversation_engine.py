...
class DeepseekConversationEngine:
    def __init__(self,role=None):
        ...
        :return: True
        """
        self.__DEEPSEEK_API_KEY = os.environ['DEEPSEEK_API_KEY']    # 从系统环境变量中读入密钥和检查密钥
        """-----------------------------------------------------核心业务-----------------------------------------------------"""
    ...
    def ask(self, ask_content, out = False):
        ...
        try:
            ...
            if not self.stream: # 非流式输出记录
                ...
                if out: print(f"AI:{assistant_content}")        # 打印回答结果
                # 存在思维链才收集
                if hasattr(response.choices[0].message, "reasoning_content"):
                    self.reasoning_content = response.choices[0].message.reasoning_content
                    if out: print(f"AI思维链:{response.choices[0].message.tool_calls}")
